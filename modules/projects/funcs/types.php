 ...
 * @Createdate Wed, 07 Feb 2018 04:04:12 GMT
 */

if( ! defined( 'NV_IS_MOD_PROJECTS' ) ) die( 'Stop!!!' );
...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( empty( $error ) )
	{
		try
		{
			...
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_types SET title = :title, note = :note, active = :active WHERE id=' . $row['id'] );
			}
