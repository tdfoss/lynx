...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    $addtime = $db->query('SELECT addtime FROM ' . NV_PREFIXLANG . '_' . $module_data . '_workreport WHERE id=' . $id)->fetchColumn();

    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id']) and nv_check_action($addtime)) {
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_workreport  WHERE id = ' . $db->quote($id));
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    } elseif (empty($row['id']) && $db->query('SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_workreport WHERE fortime=' . $row['fortime'] . ' AND userid=' . $user_info['userid'])->fetchColumn() > 0) {
        $error[] = $lang_module['error_required_fortime'];
    }
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                $stmt = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_workreport (userid, fortime, content, addtime) VALUES (' . $user_info['userid'] . ', :fortime, :content, ' . NV_CURRENTTIME . ')');
            } else {
                $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_workreport SET fortime = :fortime, content = :content WHERE id=' . $row['id']);
            }
    ...
    $row = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_workreport WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
...
if (defined('NV_IS_GODADMIN')) {
    $result = $db->query('SELECT t1.userid, username, last_name, first_name FROM ' . NV_USERS_GLOBALTABLE . ' t1 INNER JOIN ' . NV_USERS_GLOBALTABLE . '_groups_users t2 ON t1.userid=t2.userid WHERE t2.group_id IN (' . $array_config['workgroup'] . ')');
...
$db->sqlreset()
    ->select('COUNT(*)')
    ->from(NV_PREFIXLANG . '_' . $module_data . '_workreport')
...
while ($view = $sth->fetch()) {
    ...
    if (nv_check_action($view['addtime'])) {
        $allow_action = 1;
        $view['link_edit'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;id=' . $view['id'] . '&amp;redirect=' . nv_redirect_encrypt($client_info['selfurl']);
...
if (defined('NV_IS_GODADMIN') && !empty($array_users)) {
...
if (!$global_config['rewrite_enable']) {
    ...
    $xtpl->assign('ACTION', nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op, true));
}
if (!$global_config['rewrite_enable']) {
