 ...
 * @Createdate Wed, 01 Aug 2018 01:40:04 GMT
 */
...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
    ...
    if( empty( $error ) )
    {
        try
        {
            if( empty( $row['id'] ) )
            {
                $stmt = $db->prepare( 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_cat (title, note, weight) VALUES (:title, :note, :weight)' );
            ...
            {
                $stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_cat SET title = :title, note = :note WHERE id=' . $row['id'] );
            }
