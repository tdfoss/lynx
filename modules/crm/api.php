...
require_once NV_ROOTDIR . '/modules/crm/language/' . NV_LANG_DATA . '.php';
...
$app->post('/api/customer/add/', function (Request $request, Response $response, array $args) {
    ...
    if (is_file(NV_DOCUMENT_ROOT . $row['image'])) {
        $row['image'] = substr($row['image'], strlen(NV_BASE_SITEURL . NV_UPLOADS_DIR . '/crm/'));
    } else {
        $row['image'] = '';
    }
...
$app->get('/api/customers/', function (Request $request, Response $response, array $args) {
    global $db;
    $customers = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_crm_customer')
        ->fetchAll();
...
$app->get('/api/customer/{id}/', function (Request $request, Response $response, array $args) {
    global $db;
    $id = $request->getAttribute('id');
    $customer = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_crm_customer WHERE id=' . $id)
        ->fetch();
