...
$table_name = NV_PREFIXLANG . '_' . $module_data . '_events_type';
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    try {
        if (empty($row['id'])) {
            $stmt = $db->prepare('INSERT INTO ' . $table_name . ' (title, note, weight) VALUES (:title, :note, :weight)');
            $weight = $db->query('SELECT max(weight) FROM ' . $table_name . '')->fetchColumn();
            $weight = intval($weight) + 1;
            $stmt->bindParam(':weight', $weight, PDO::PARAM_INT);
        } else {
            $stmt = $db->prepare('UPDATE ' . $table_name . ' SET title = :title, note = :note WHERE id=' . $row['id']);
        }
