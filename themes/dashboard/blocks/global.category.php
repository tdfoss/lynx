...
if (! nv_function_exists('nv_menu_category')) {
    ...
    function nv_menu_category($block_config)
    {
        ...
        $xtpl = new XTemplate('global.category.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
     ...
     * @return string
     */
    function nv_get_category_submenu($id, $array_menu, &$submenu_active, $block_theme, $template)
    {
        $xtpl = new XTemplate('global.category.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
        ...
        if (! empty($array_menu[$id])) {
            foreach ($array_menu[$id] as $sid => $smenu) {
                ...
                if (isset($array_menu[$sid])) {
                    $submenu = nv_get_category_submenu($sid, $array_menu, $submenu_active, $block_theme, $template);
                    $xtpl->assign('SUB', $submenu);
