	...
	<div class="table-responsive">
		<table class="table table-striped table-bordered table-hover">
			...
			<tbody>
				...
				<tr>
					<td>
						<select class="form-control" id="id_weight_{VIEW.tid}" onchange="nv_change_weight('{VIEW.tid}');">
						<!-- BEGIN: weight_loop -->
...
//<![CDATA[
	function nv_change_weight(id) {
		...
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=customer_tag&nocache=' + new Date().getTime(), 'ajax_action=1&tid=' + id + '&new_vid=' + new_vid, function(res) {
			var r_split = res.split('_');
			if (r_split[0] != 'OK') {
				alert(nv_is_change_act_confirm[2]);
			}
			window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=customer_tag';
			return;
		});
		return;
	}
