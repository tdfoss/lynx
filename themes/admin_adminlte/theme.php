...
function nv_admin_theme($contents, $head_site = 1)
{
    ...
    if ($head_site == 1) {
        ...
        foreach ($admin_menu_mods as $m => $v) {
            $xtpl->assign('MENU_CLASS', (($module_name == $m) ? 'active' : ''));
            $xtpl->assign('MENU_HREF', $m);
            $xtpl->assign('MENU_NAME', $v);
            $xtpl->assign('MODULE_ICON', isset($array_awesome_icon[$m]) ? $array_awesome_icon[$m] : 'globe');
            ...
            if ($m != $module_name) {
                ...
                foreach ($submenu as $n => $l) {
                    ...
                    $xtpl->assign('MENU_SUB_CURRENT', (((!empty($op) and $op == $n) or (!empty($set_active_op) and $set_active_op == $n)) ? 'active' : ''));
                    $xtpl->assign('MENU_SUB_HREF', $m);
                    $xtpl->assign('MENU_SUB_OP', $n);
                    ...
                    if (!empty($_submenu_i)) {
                        foreach ($_submenu_i as $sn => $sl) {
                            ...
                            $xtpl->parse('main.menu_loop.current.loop.submenu.loop');
                        }
                        $xtpl->parse('main.menu_loop.current.loop.submenu');
                    }
                    $xtpl->parse('main.menu_loop.current.loop');
                }
